...
export function GET(req: NextRequest) {
  ...
  const category = (searchParams.get('category') as 'all' | 'lp' | 'equipment' | 'artist' | 'other' | null) ?? 'all'
  const page = Number(searchParams.get('page') ?? '1')
  const pageSize = Number(searchParams.get('pageSize') ?? '10')
  const data = listReviews({ q, status, category, page: Number.isFinite(page) ? page : 1, pageSize: Number.isFinite(pageSize) ? pageSize : 10 })
...
export async function POST(req: NextRequest) {
  ...
  const created = createReview({
    ...
    category: (body.category as 'lp' | 'equipment' | 'artist' | 'other') ?? 'lp',
