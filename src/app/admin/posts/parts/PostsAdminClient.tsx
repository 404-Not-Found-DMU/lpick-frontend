    ...
    <div>
      ...
      <DataTable
        columns={[
          ...
          { key: 'title', header: '제목', span: 6, render: (_, r) => <Link className="text-violet-600 hover:underline block truncate" href={`/community/${r.id}`}>{r.title}</Link> },
          ...
          { key: 'reportedCount', header: '신고', className: 'text-center', headerClassName: 'text-center', span: 1, render: (v) => v ? <span className="rounded-full bg-rose-100 px-2 py-0.5 text-xs text-rose-700">{v}</span> : <span className="text-gray-400">-</span> },
