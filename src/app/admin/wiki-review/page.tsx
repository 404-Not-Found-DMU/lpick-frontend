...
export default async function AdminWikiReviewPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  const category = ((sp.category as string) as 'all' | 'lp' | 'equipment' | 'artist' | 'other') ?? 'all'
  const page = Number((sp.page as string) ?? '1')
  const pageSize = Number((sp.pageSize as string) ?? '10')
  const { items, total } = listReviews({ q, status, category, page: Number.isFinite(page) ? page : 1, pageSize: Number.isFinite(pageSize) ? pageSize : 10 })
  return (
    <div className="space-y-6">
      ...
      <WikiReviewAdminClient items={items} total={total} q={q} status={status} category={category} page={Number.isFinite(page) ? page : 1} pageSize={Number.isFinite(pageSize) ? pageSize : 10} />
    </div>
  )
}
